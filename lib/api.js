...
exports.getChallenge = function(slugOrId, save) {
  ...
  return makeRequest('GET', format('/code-challenges/%s', slugOrId))
    .then(function(data) {
...
function buildCodeFile(slug) {
  ...
  var body = train.session.code || train.session.setup;
...
exports.handleTrainResponse = function(data) {
  ...
  return this.getChallenge(kata.slug)
    ...
    .then(function() {
      ...
      modeler.renderTrainingSession(kata.slug);
      modeler.openWithEditor(kata.slug);
      return true;
    });
};
...
exports.trainNext = function(strategy) {
  ...
  var route = format('/code-challenges/%s/train', config.language);
  var method = 'POST';
  //var postData = format('strategy=%s', strategy || 'default');
  var postData = {strategy: strategy || 'default'};
...
exports.train = function(slugOrId) {
  ...
  var route = format('/code-challenges/%s/%s/train', slugOrId, config.language);
  var method = 'POST';
  //var postData = format('language=%s', config.language);
  var postData = {language: config.language};
  var handleResponse = this.handleTrainResponse.bind(this);
...
exports.attemptSolution = function(slugOrId) {
  ...
  var code = codeFile.substr(markerLine + 1);

  var route = format('/code-challenges/projects/%s/solutions/%s/attempt', projectId, solutionId);
  var method = 'POST';
  var postData = {code: code, output_format: 'raw'};

  var handleDeferred = this.getDeferredResponse;
  ...
  return makeRequest(method, route, postData)
    ...
    .then(function(result) {
      ...
      if (evaluatedMessage.valid === true) {
        ...
        modeler.renderReadyToFinalizeMessage(lookup);
        return;
      }
...
exports.finalizeSolution = function(slugOrId) {
  ...
  var route = format('/code-challenges/projects/%s/solutions/%s/finalize', projectId, solutionId);
  var method = 'POST';
