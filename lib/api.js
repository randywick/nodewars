 ...
 * @param {string=} postData  the request body, if any, to be sent with an
*                             https POST request.
 *
 ...
 * @returns {Promise}
 */
exports.getChallenge = function(slugOrId) {
  return makeRequest('GET', format('/%s', slugOrId))
    .then(function(data) {
      console.log(data);
      var kata = JSON.parse(data);
      reference.setData(kata.id, kata.slug);
...
/**
 * @private
 * Provided with data from {@link trainNext} or {@link startSession},
 * updates train.json, session.json and kata.js
 *
 * @param {string} data response from {@link trainNext} or {@link startSession}
 *
 * @returns {Promise}
 */
 ...
 */
exports.startSession = function(slugOrId) {
...
exports.attemptSolution = function(slugOrId) {
  ...
  return makeRequest(method, route, postData)
    ...
    .then(function(result) {
      ...
      return handleDeferred(obj.dmid);
    })

    .then(function(result) {
      var obj = JSON.parse(result);
      console.log('Response received.  Valid: %s', obj.valid);
      writeKataFile(lookup.slug, 'result.json', obj);
    });
...
exports.finalizeSolution = function(slugOrId) {
  ...
  return makeRequest(method, route)
    .then(function() { console.log('awe yeah.'); });
};