...
exports.renderReadyToFinalizeMessage = function(lookup) {
  ...
  console.log(chalk.white('  nodewars finalize ' + lookup.slug));
};
...
exports.renderExerciseCompleteMessage = function(kata) {
  console.log( '%s was successfully %s', data.name, chalk.green('finalized'));
  console.log(
  ...
  console.log(chalk.white('  nodewars edit ' + data.slug));
  console.log('');

  console.log('Or you may train on the record again by typing:');
  console.log(chalk.white('  nodewars train ' + data.slug));
};
...
exports.openWithEditor = function(slugOrId) {
  ...
  var challengeDir = path.resolve(config.projectDir, lookup.slug);
  ...
  var self = this;
  var child = spawn(config.editor, [challengeDir]);
  child.on('close', function(code) {
    console.log('Editor process terminated.');
    console.log('');
    self.renderChallengeList();
  })
